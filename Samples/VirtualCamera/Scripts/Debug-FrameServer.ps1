...
{
    Write-Log "Caught Exception! $message"      -logStyle Error -stackIdx 2
...
function Debug-Service_Interal
{
    ...
    if([String]::IsNullOrEmpty($windbg) -eq $true)
    {
        ...
        {
            Write-Log "Cannot find windbg on system please provides the installed path -windbg [installpath]\windbg.exe" -logStyle Error
    ...
    {  
        ...
        $debugFSJob = Start-Job -Name $debugFSJob_Name -ArgumentList @($windbgPath, $timeout, $serviceName) -InitializationScript $initScriptBlock  -ScriptBlock {
            ...
            try 
            {
                ...
                $fixTimeout = 10;
                $loopCount = [math]::floor($timeout_s / $fixTimeout);
            ...
            {
                Log-Exception "[debugFS] Error trying to debug frameserver: $_"
                return $processId
            }
            return $processId
        }
