    ...
    _Outptr_ IMFVirtualCamera** ppVirtualCamera)
{
    ...
    if(!strRealCamSymLink.empty())
    {
        LOG_COMMENT(L"Add real cam: %s \n", strRealCamSymLink.data());
        RETURN_IF_FAILED(spVirtualCamera->AddDeviceSourceInfo(strRealCamSymLink.data()));
    }
    ...
    catch (...) { LOG_WARN(L"not running in app pacakge\n"); }
...
HRESULT VCamUtils::GetRealCameras(std::vector<DeviceInformation>& realCameraList)
{
    realCameraList.clear();
