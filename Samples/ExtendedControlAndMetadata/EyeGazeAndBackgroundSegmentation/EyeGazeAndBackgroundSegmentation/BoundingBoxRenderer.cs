...
{
    /// <summary>
    /// Helper class to render bounding boxes
    /// </summary>
    internal class BoundingBoxRenderer
    {
        ...
        public BoundingBoxRenderer(Canvas canvas, int maxBoxes = 50, int lineThickness = 6)
        {
        ...
        /// <summary>
        /// Render bounding boxes with optional labels
        /// </summary>
        /// <param name="boundingRectangles"></param>
        public void Render(IReadOnlyList<Rect> boundingRectangles, IReadOnlyList<string> messages)
        {
            int i = 0;
            m_rawRects.Clear();
            // Render detections up to MAX_BOXES
            for (i = 0; i < boundingRectangles.Count && i < m_rectangles.Length; i++)
            {
                // Cache rect
                m_rawRects.Add(boundingRectangles[i]);

                // Render bounding box
                m_rectangles[i].Width = boundingRectangles[i].Width * m_canvas.ActualWidth;
                m_rectangles[i].Height = boundingRectangles[i].Height * m_canvas.ActualHeight;
                Canvas.SetLeft(m_rectangles[i], boundingRectangles[i].X * m_canvas.ActualWidth);
                Canvas.SetTop(m_rectangles[i], boundingRectangles[i].Y * m_canvas.ActualHeight);
                m_rectangles[i].Visibility = Visibility.Visible;

                // Render text label
                m_textBlocks[i].Text = messages[i];
                Canvas.SetLeft(m_borders[i], boundingRectangles[i].X * m_canvas.ActualWidth + 2);
                Canvas.SetTop(m_borders[i], boundingRectangles[i].Y * m_canvas.ActualHeight + 2);
