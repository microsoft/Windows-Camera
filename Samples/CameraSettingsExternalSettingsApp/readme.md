...
This sample app will demonstrate how to build an external camera settings app (also known as a "***[camera companion app](https://learn.microsoft.com/en-us/windows-hardware/drivers/stream/camera-companion-apps)***") associated with a camera device driver. This app allows to adjust control in real time as well as set a control value as the default one to be applied every time the associated camera is started. For example, a companion app has the privilege to prescribe that *VideoHDR* should be turned ON by default when using the camera, assuming *VideoHDR* is a supported control in the first place. The association between a companion app and a camera device is typically embedded by the camera driver at installation time in its *.inf* (i.e. by a camera manufacturer wanting to offer an app tie-in to their physical device or by a virtual camera also bundling a companion app that instantiates the virtual camera such as [the ***VirtualCameraManager_App*** in our other sample](..\VirtualCamera\README.md)). This companion app would then be launchable directly from the associated camera device settings page *(Windows Settings &rarr; Bluetooth & devices &rarr; Cameras &rarr; \<the associated camera\> in the "Related settings" section at the bottom)*
